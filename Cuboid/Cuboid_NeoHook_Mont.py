...
Implements the Cuboid Hyperelastic models (Neo-Hookean)
"""

from dem_hyperelasticity.Cuboid import define_structure as des
...
from dem_hyperelasticity.Cuboid import config as cf
...
class DeepEnergyMethod:
    ...
    def train_model(self, data, bForce, neumannBC, dirichletBC, LHD, iteration, learning_rate):
        ...
        for t in range(iteration):
            ...
            def closure():
                ...
                internal2 = self.intLoss.montecarlo3D(storedEnergy, LHD[0], LHD[1], LHD[2])
                body_force = torch.from_numpy(bForce).float().to(dev)
                bF = torch.bmm((u_pred + x).unsqueeze(1), body_force.unsqueeze(2))
                body_force_crit = self.intLoss.montecarlo3D(bF, LHD[0], LHD[1], LHD[2])
                ...
                for i, vali in enumerate(neuBC_coordinates):
                    ...
                    if i == 0:
                        external2[i] = self.intLoss.montecarlo2D(fext, LHD[0], LHD[2])
                    elif i == 1:
                        external2[i] = self.intLoss.montecarlo2D(fext, LHD[0], LHD[1])
                    elif i == 2:
                        external2[i] = self.intLoss.montecarlo2D(fext, LHD[0], LHD[2])
                    elif i == 3:
                        external2[i] = self.intLoss.montecarlo2D(fext, LHD[0], LHD[1])
                ...
                energy_loss = internal2 - body_force_crit - torch.sum(external2)
    ...
    def getU(self, x):
        ...
        length = 1.25
        Ux_known = 0
        Uy_known = factor * (0.5 + (x[:, 1] - 0.5) * torch.cos(angle) - (x[:, 2] - 0.5) * torch.sin(angle) - x[:, 1])
        Uz_known = factor * (0.5 + (x[:, 1] - 0.5) * torch.sin(angle) + (x[:, 2] - 0.5) * torch.cos(angle) - x[:, 2])
        Ux = Ux_known                      + (x[:, 0] - length) * x[:, 0] * u[:, 0]
        Uy = (Uy_known * x[:, 0] / length) + (x[:, 0] - length) * x[:, 0] * u[:, 1]
        Uz = (Uz_known * x[:, 0] / length) + (x[:, 0] - length) * x[:, 0] * u[:, 2]
...
if __name__ == '__main__':
    ...
    dem = DeepEnergyMethod([cf.D_in, cf.H, cf.D_out], 'montecarlo', mat, 3)
    # ----------------------------------------------------------------------
    #                   STEP 3: TRAINING MODEL
    # ----------------------------------------------------------------------
    start_time = time.time()
    cf.iteration = 25
    cf.filename_out = "./output/dem/NeoHook_3Layer_lr0p5_mesh41x41x41_iter25_mont"
    body_force = cf.np.ones(dom.shape) * cf.np.array([cf.body_force_x, cf.body_force_y, cf.body_force_z])
    dem.train_model(dom, body_force, boundary_neumann, boundary_dirichlet, [cf.Length, cf.Height, cf.Depth], cf.iteration, cf.lr)
