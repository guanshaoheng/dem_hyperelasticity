...
class DeepEnergyMethod:
    ...
    def train_model(self, data1, data2, neumannBC, dirichletBC, iteration, learning_rate):
        ...
        for t in range(iteration):
            ...
            def closure():
                ...
                volume = (1 * 1 * 6)
                internalNN1 = volume * self.loss_sum(storedEnergy1)
                u_pred2 = self.getU(x2)
                u_pred2.double()
                storedEnergy2 = self.energy.getStoredEnergy(u_pred2, x2)
                volume = (3 * 1 * 1)
                internalNN2 = volume * self.loss_sum(storedEnergy2)
                ...
                for i, vali in enumerate(neuBC_coordinates):
                    ...
                    externalNN[i] = area * self.loss_sum(torch.bmm((neu_u_pred + neuBC_coordinates[i]).unsqueeze(1), neuBC_values[i].unsqueeze(2)))
...
if __name__ == '__main__':
    ...
    start_time = time.time()
    cf.iteration = 25
    cf.filename_out = "./output/dem/NeoHook_3Layer_lr0p5_mesh10x10x60-30x10x10_iter25_mont"
    dem.train_model(dom1, dom2, boundary_neumann, boundary_dirichlet, cf.iteration, cf.lr)
