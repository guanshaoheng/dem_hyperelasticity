...
class DeepEnergyMethod:
    ...
    def __init__(self, model, numIntType, energy, dim):
        # self.data = data
        self.model = MultiLayerNet_curl(model[0], model[1], model[2], num_layers=cf.num_layers)
    ...
    def train_model(self, shape, dxdydz, data, neumannBC, dirichletBC, iteration, learning_rate):
        ...
        for i, keyi in enumerate(dirichletBC):
            dirBC_coordinates[i] = torch.from_numpy(dirichletBC[keyi]['coord']).float().to(dev).requires_grad_(True)
        ...
        boundary_loss_array = []
        loss_array = []
        loss_min, trial_num, patience = 1e9, 0, 50
        ...
        for t in range(iteration):
            ...
            for i, vali in enumerate(neuBC_coordinates): # 遍历力边界条件
                ...
                # fext_original = torch.bmm((neu_u_pred + neuBC_coordinates[i]).unsqueeze(1), neuBC_values[i].unsqueeze(2))
                fext = torch.einsum("nj, nj->n", neu_u_pred , neuBC_values[i])
            ...
            line = 'Iter: %d Loss: %.9e Internal: %.9e External: %.9e Boundary: %.9e Time: %.3e mins %s' \
                % (t + 1, loss.item(), internal2.item(), external_total.item(), 
                boundary_loss.item(), (time.time() - it_time)/60., "Impd." if improved_flag else "NoImp.")
            print(line)
    ...
    def evaluate_model_degraded(self, x, y, z):
        ...
        E = 0.5 *(torch.einsum("nji, njk->nik", F, F) - torch.eye(3, device=dev, dtype=torch.float32).view(1, 3, 3).repeat(Nx*Ny*Nz, 1, 1))
        E_cauchy = 0.5 * (F +  torch.einsum("nij->nji", F))  - torch.eye(3, device=dev, dtype=torch.float32).view(1, 3, 3).repeat(Nx*Ny*Nz, 1, 1)
...
if __name__ == '__main__':
    ...
    # ----------------------------------------------------------------------
    start_time = time.time()
    filename_out = f"{cf.filename_out:s}"
    ...
    U, S11, S12, S13, S22, S23, S33, E11, E12, E13, E22, E23, E33, \
        ...
        E11_cauchy, E12_cauchy, E13_cauchy, E22_cauchy, E23_cauchy, E33_cauchy = dem.evaluate_model_degraded(x, y, z)

    util.write_vtk_v2(os.path.join(filename_out, "deformation"),
