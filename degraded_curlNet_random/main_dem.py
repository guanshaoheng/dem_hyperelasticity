...
def output_results(epoch):
    ...
    write_to_vtk(
        ...
        E11_cauchy, E12_cauchy, E13_cauchy, E22_cauchy, E23_cauchy, E33_cauchy,
        random_health_field.reshape(Nx, Ny, Nz)
        )
    surUx, surUy, surUz = U
...
if __name__ == '__main__':
    ...
    num_test_z = Nz
    # ------------------------------ filename output ----------------------------------------------------
    out_dir = "simu_curlnet_random_tx"
    ...
    # ----------------------------------------------------------------------
    for random_healthy_index in range(1, 2):
    
        filename_out = f"{out_dir:s}/{model_energy_type:s}_{integration_method:s}_{int(Nx):d}x{int(Ny):d}x{int(Nz):d}_" + \
            f"theta{theta_ratio:.1f}_phi{phi_ratio:.1f}_K{K_penalty:.1e}_mu{mu:.1e}"+\
            f"_layers{num_layers:d}_leftPenalty{bc_left_penalty:.0e}_helthRandom{random_healthy_index:d}_tx{known_right_tx:.1e}"
        ...
        random_health_field = get_random_field(coords=dom, random_index=random_healthy_index)
        ...
        mat = md.EnergyModel(
            ...
            healthy_ratio=0.,
            random_health_flag=True, 
            random_health_field = random_health_field)
