...
Implements the 2D Hyperelastic beam models (Mooney-Rivlin)
"""
...
class DeepEnergyMethod:
    ...
    def evaluate_model(self, x, y, z):
        ...
        if energy_type == 'mooneyrivlin':
            if self.dim == 2:
                c1 = self.energy.param_c1
                c2 = self.energy.param_c2
                c = self.energy.param_c
                d = self.energy.param_d
                Nx = len(x)
                Ny = len(y)
                xGrid, yGrid = np.meshgrid(x, y)
                x1D = xGrid.flatten()
...
if __name__ == '__main__':
    ...
    #                   STEP 2: SETUP MODEL
    # ----------------------------------------------------------------------
    mat = md.EnergyModel('mooneyrivlin', 2, param_c1=cf.param_c1, param_c2=cf.param_c2, param_c=cf.param_c)
    ...
    dxdy = [cf.hx, cf.hy]
    cf.iteration = 80
    cf.filename_out = "./output/dem/MooneyRivlin_2Layer_mesh100x25_iter80_trap"
