...
class DeepEnergyMethod:
    ...
    def train_model(self, shape, dxdydz, data, neumannBC, dirichletBC, iteration, learning_rate):
        ...
        for i, keyi in enumerate(dirichletBC):
            ...
            dirBC_direction[i] = torch.tensor(dirichletBC[keyi]["direction"], dtype=torch.int).to(self.device)
        # -------------------------------------------------------------------------------
        ...
        for t in range(iteration):
            ...
            for i, vali in enumerate(dirBC_coordinates): # 遍历位移边界条件
                ...
                bc_u_crit[i] = self.loss_squared_sum(dir_u_pred[..., dirBC_direction[i]], dirBC_values[i][..., dirBC_direction[i]]) * dirBC_penalty[i]
        ...
        # get the displacement in the end of right side
        u_pre_right = self.getU(dirBC_coordinates[1])
        averaged_u_pre_right_x = torch.mean(u_pre_right, dim=0).cpu().detach().numpy()
        ...
        loadind_infor = f"[lx, ly, lz] = [{lx:.4e}, {ly:.4e}, {lz:.4e}]\n"\
            f"[target_x, target_y, target_z] = [{known_right_ux:.4e}, {known_right_uy:.4e}, {known_right_uz:.4e}]\n"\
            f"[dx, dy, dz]=[{averaged_u_pre_right_x[0]:.4e}, {averaged_u_pre_right_x[1]:.4e}, {averaged_u_pre_right_x[2]:.4e}]\n"
        line+= (loadind_infor + "#"*80 + "\n")
        print(line)
        f_outstream.writelines(line)
        f_outstream.close()
        ...
        # -------------------- save the model --------------------
        model_saved_path = f"{filename_out}/trained_model.pt"
        torch.save(self.model, model_saved_path)
        print(f"Network model saved as: {model_saved_path}")
        print()
...
if __name__ == '__main__':
    ...
    known_left_uy = 0
    known_left_uz = 0
    known_right_ux = lx * 0.5
    known_right_uy = 0
    known_right_uz = 0
    bc_left_penalty = 1000000.  # 1e6
    ...
    num_test_z = Nz
    # ------------------------------ filename output ------------------------------
    out_dir = "normalNet_displacement"
    ...
    # ------------------------------
    for bc_left_penalty in [1e9]:  # 1e2, 1e3, 1e4, 1e5, 1e6, 1e7, 1e8, 1e9
    # ------------------------------
    ...
    # for health_coefficient in np.linspace(0, 1.0, 6):
        ...
        filename_out = f"{out_dir:s}/{model_energy_type:s}_{integration_method:s}_{int(Nx):d}x{int(Ny):d}x{int(Nz):d}_" + \
            ...
            f"_layers{num_layers:d}_leftPenalty{bc_left_penalty:.0e}_helth{health_coefficient:.1f}_tx{known_right_tx:.1e}_test"
